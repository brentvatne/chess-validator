  ...
  module Rules
    ...
    def path_for(board, origin, destination)
      ...
      if delta_row.abs == delta_col.abs
        #diagonal
        (origin.row..destination.row).each do |n|
          paths << Coordinates.new(n, n) unless n == destination.row
        end
      elsif delta_col == 0
        #vertical
        (origin.row..destination.row).each do |n|
          paths << Coordinates.new(origin.row, n) unless n == destination.column
        end
      elsif delta_row == 0
        #horizontal
        (origin.column..destination.column).each do |n|
          paths << Coordinates.new(n, origin.column) unless n == destination.row
        end
      end
